...
class DQN:
    def __init__(self, n_states, n_actions):
        ...
        self.memory = np.zeros((20000, 2 * 2 + 2))  # 2*2(state和next_state,每个x,y坐标确定)+2(action和reward),存储2000个记忆体
        self.cost = []  # 记录损失值
    ...
    def learn(self):
        ...
        # 使用记忆库中批量数据
        sample_index = np.random.choice(2000, 1024)  # 2000个中随机抽取32个作为batch_size
